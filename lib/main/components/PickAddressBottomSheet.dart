...
import 'package:gogo_riders_delivery/user/screens/CreateOrderScreen.dart';
...
class PickAddressBottomSheetState extends State<PickAddressBottomSheet> {
  bool _serviceEnabled = false; // Variable to hold location service status

  @override
  void initState() {
    super.initState();
    init();
    checkLocationService(); // Check location service status when the widget initializes
  }

  // Method to check location service status
  Future<void> checkLocationService() async {
    _serviceEnabled = await Geolocator.isLocationServiceEnabled();

    if (!_serviceEnabled) {
      toast("Location Turned Off. Please Enable Location Services");
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            if (_serviceEnabled) {
              // Check if location service is enabled before launching GoogleMapScreen
